...
class IntegerArgumentTest extends ArgumentTestBase {

	@Test
	@DisplayName("Should correctly match integer inputs")
	void testMatches() {
		...
		assertFalse(argument.matches(createToken("3.14")), "Input '3.14' should not match expected format (not an integer)");
		assertFalse(argument.matches(createToken("not-a-number")), "Input 'not-a-number' should not match expected format (not an integer)");
	...
	void testIsValid() {
		...
		assertFalse(argument.isValid(createToken("2.72")), "Input '2.72' should be invalid (not an integer)");
	...
	void testIsValidWithBounds() {
		final IntegerArgument argument = IntegerArgument.create("bounded-integer")
				.withBounds(10, 100)
				.build();
	...
	@Test
	@DisplayName("Should correctly parse valid integer inputs")
	void testParseValid() {
		final IntegerArgument argument = IntegerArgument.create("integer")
				.build();
	...
	@Test
	@DisplayName("Should throw exception when parsing invalid integer inputs")
	...
	@Test
	@DisplayName("Should correctly parse integer inputs within bounds")
	...
	@Test
	@DisplayName("Should throw exception when parsing integer inputs out of bounds")
